...
class GateWaysApiService {
  final http.Client _httpClient;

  GateWaysApiService({http.Client? httpClient})
      : _httpClient = httpClient ?? http.Client();

  Future<GatewaysModel> getGateWaysData() async {
    final responseBody = await _getData();

    try {
      //
      return GatewaysModel.fromJson(responseBody);
      //
    } catch (exception, stackTrace) {
      //
      printLog(
        classFileName: 'GateWaysApiService',
  ...
  Future<dynamic> _getData() async {
    ...
    try {
      ...
      return apiResponseHelper(
        response: response,
        className: 'GateWaysApiService',
        apiUrl: getGateWaysApi,
