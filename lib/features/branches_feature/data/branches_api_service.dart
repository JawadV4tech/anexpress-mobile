...
class BranchesApiService {
  ...
  BranchesApiService({http.Client? httpClient})
      : _httpClient = httpClient ?? http.Client();

  Future<List<BankBranchInfoModel>> getBanksData({required int bankId}) async {
    final responseBody = await _getData(bankId: bankId);

    try {
      //
      return (responseBody['banksList'] as List)
          .map((i) => BankBranchInfoModel.fromJson(i))
          .toList();
      //
    } catch (exception, stackTrace) {
      //
      printLog(
        classFileName: 'BranchesApiService',
  ...
  /// get Data Functions
  Future<dynamic> _getData({required int bankId}) async {
    final token = await SharedPreferencesHelpers()
        .getStringData(key: PrefKeys.authTokenPrefKey);

    final branchesApiUrl = getBranchesApi(bankId: bankId);
    try {
      final response = await _httpClient.get(
        Uri.parse(branchesApiUrl),
      ...
      return apiResponseHelper(
        response: response,
        className: 'BranchesApiService',
        apiUrl: branchesApiUrl,
